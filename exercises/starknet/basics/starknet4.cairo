...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32 ) {
            let call = get_caller_address();

            assert(caller == self.contract_owner.read(), 'caller not owner');
        ...
        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            ...
            let current_stock = self.invevntory.read(product);
            assert(current_stock >= quantity, 'Insufficent stock');
